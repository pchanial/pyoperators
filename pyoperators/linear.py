...
class DiagonalNumexprNonSeparableOperator(DiagonalOperator):
    ...
    def __init__(self, expr, global_dict=None, broadcast=None, dtype=float, **keywords):
        if not isinstance(expr, str):
        ...
        BroadcastingOperator.__init__(self, None, broadcast, dtype=dtype, **keywords)
    ...
    def __init__(
        ...
        **keywords,
        ...
        if convention == 'X':
            ...
            m = gm(c2, -s2, 0, c1 * s2, c1 * c2, -s1, s1 * s2, c2 * s1, c1)
        elif convention == "XY'":
            m = gm(c2, 0, s2, s1 * s2, c1, -c2 * s1, -c1 * s2, s1, c1 * c2)
        elif convention == "YX'":
            m = gm(c1, s1 * s2, c2 * s1, 0, c2, -s2, -s1, c1 * s2, c1 * c2)
        elif convention == "YZ'":
            m = gm(c1 * c2, -c1 * s2, s1, s2, c2, 0, -c2 * s1, s1 * s2, c1)
        elif convention == "ZY'":
            m = gm(c1 * c2, -s1, c1 * s2, c2 * s1, c1, s1 * s2, -s2, 0, c2)
        elif convention == "ZX'":
            m = gm(c1, -c2 * s1, s1 * s2, s1, c1 * c2, -c1 * s2, 0, s2, c2)
