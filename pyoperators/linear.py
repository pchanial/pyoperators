...
from .core import (Operator, BlockRowOperator, BroadcastingOperator,
                   ...
                   ReductionOperator, DirectOperatorFactory,
...
class EinsteinSumOperator(Operator):
    ...
    def __init__(self, subscripts, *operands, **keywords):
        ...
        self.lsubscripts = tuple([self._get_subscript_as_list(s)
                                  for s in ins + [out]])
    ...
    def reshapein(self, shapein):
        shapes = [None if isinstance(o, Variable) else o.shape
                  for o in self.operands] + [None]
        shapes[self.iX] = shapein
        return self._get_shape(shapes, -1)

    def reshapeout(self, shapeout):
        shapes = [None if isinstance(o, Variable) else o.shape
                  for o in self.operands] + [shapeout]
    ...
    def _get_reversed_subscripts(self):
        ...
        ls_reversed = [''.join(['...' if a is Ellipsis else a for a in l])
                       for l in ls_reversed]
...
class SymmetricBandToeplitzOperator(Operator):
    ...
    def direct(self, x, out):
        with _pool.get(self.fftsize, self.dtype, aligned=True,
                       contiguous=True) as rbuffer:
            with _pool.get(
                self.fftsize // 2 + 1, complex_dtype_for(self.dtype),
                aligned=True, contiguous=True) as cbuffer:
