...
class Operator(object):
    ...
    def _set_inout(self, shapein, shapeout, reshapein, reshapeout,
        ...
        if shapein is not None and (shapein == shapeout or \
           shapeout is None and self._reshapein is None):
            self.flags = self.flags._replace(SQUARE=True)
...
class DiagonalOperator(BroadcastingOperator):
    ...
    def __init__(self, data, broadcast='disabled', **keywords):
        ...
        self.add_rule('{DiagonalOperator}.', lambda o: self._rule_diagonal(o,
                      np.add), AdditionOperator)
        self.add_rule('{DiagonalOperator}.', lambda o: self._rule_diagonal(o,
                      np.multiply))
    ...
    def _rule_diagonal(self, d, operation):
        ...
        return DiagonalOperator(data, broadcast, shapein=shapein, dtype= \
...
class ZeroOperator(ScalarOperator):
    ...
    def __init__(self, shapein=None, shapeout=None, dtype=None, reshapein=None,
                 reshapeout=None, **keywords):
        if shapein is not None and shapein == shapeout or reshapein is None and\
           reshapeout is None:
            flags = {'SQUARE':True, 'SYMMETRIC': True, 'IDEMPOTENT': True}
        else:
            flags = None
        ScalarOperator.__init__(self, 0, shapein=shapein, shapeout=shapeout,
                                  reshapein=reshapein, reshapeout=reshapeout,
                                  dtype=dtype, flags=flags, **keywords)
