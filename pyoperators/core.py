...
class OperatorRule(object):
    ...
    def _split_subject(cls, subject):
        ...
        if isinstance(subject, tuple):
            if any(not isinstance(s, str) and not issubclass(s, Operator) and
                   not isinstance(s, (list, tuple)) or
                   isinstance(s, (list, tuple)) and
                   any(not issubclass(_, Operator) for _ in s) for s in subject):
...
class CompositionOperator(NonCommutativeCompositeOperator):
    ...
    def __init__(self, operands, **keywords):
        ...
        self.set_rule(('.', type(self)), lambda s, o: CompositionOperator(
