...
class Operator(object):
    ...
    def todense(self, shapein=None, shapeout=None, inplace=False):
        ...
        if shapein is None:
            raise ValueError(
                "The operator's input shape is not explicit. Speci"
                "fy it with the 'shapein' keyword."
            )
        if shapeout is None:
            raise ValueError(
                "The operator's output shape is not explicit. Spec"
    ...
    def _init_inout(
        ...
        if isinstance(attrin, (dict, types.FunctionType, types.MethodType)):
            ...
            raise TypeError(
                "The 'attrin' keyword should be a dictionary or a f" "unction."
            )
...
class BlockSliceOperator(CommutativeCompositeOperator):
    ...
    def direct(self, input, output):
        ...
        for s, op in zip(self.slices, self.operands):
            ...
            o = output[s]
            with _pool.copy_if(
                i, op.flags.alignment_input, op.flags.contiguous_input
            ) as i:
                with _pool.copy_if(
                    o, op.flags.alignment_output, op.flags.contiguous_output
                ) as o:
                    op.direct(i, o)
...
class BlockOperator(NonCommutativeCompositeOperator):
    ...
    def _get_shape_composite(shapes, p, axis, new_axis):
        ...
        if len(explicit) != 1:
            ...
            if not all(ok):
                raise ValueError(
                    "The dimensions of the blocks '{0}' are not th"
...
class BlockRowOperator(BlockOperator):
    ...
    def direct(self, input, output):
        ...
        i = input[sins[0]]
        op = self.operands[0]
        with _pool.copy_if(i, op.flags.alignment_input, op.flags.contiguous_input) as i:
            op.direct(i, output)
...
class Variable(Operator):
    ...
    def _rule_left(self, other):
        raise ValueError(
            'A variable cannot be composed with an right-hand side' ' operator.'
        )
