...
class DiagonalOperator(BroadcastingOperator):
    def __new__(cls, data, broadcast='disabled', shapein=None, dtype=None):
        ...
        return BroadcastingOperator.__new__(
            cls, data, broadcast=broadcast, shapein=shapein, dtype=dtype
        )
...
class TridiagonalOperator(Operator):
    def __init__(self, diag, subdiag, superdiag=None, **kwargs):
        ...
        if np.iterable(subdiag) and len(subdiag) + 1 != len(diag):
            if isscalar(subdiag):
                ...
                raise ValueError(
                    "subdiagonal should be the length of the diagonal minus one or a scalar."
                )
        ...
        if np.iterable(self.superdiag) and len(self.superdiag) + 1 != len(diag):
            if isscalar(self.superdiag):
                ...
                raise ValueError(
                    "superdiagonal should be the length of the diagonal minus one."
                )
        self.dtype = np.find_common_type(
            [], [self.diag.dtype, self.subdiag.dtype, self.superdiag.dtype]
        )
        self.kwargs = kwargs
        flags = {"SYMMETRIC": np.all(self.subdiag == self.superdiag)}
        Operator.__init__(
            self, shapein=shapein, dtype=self.dtype, flags=flags, **kwargs
        )
    ...
    def associated_operators(self):
        return {
            'T': TridiagonalOperator(self.diag, self.superdiag, self.subdiag),
            'C': TridiagonalOperator(
                np.conj(self.diag), np.conj(self.subdiag), np.conj(self.superdiag)
            ),
        }
...
class EigendecompositionOperator(Operator):
    ...
    **kwargs: keyword arguments
    ...
    def __init__(self, A=None, v=None, w=None, **kwargs):
        ...
        self.eigenvectors = v
        self.kwargs = kwargs
        Operator.__init__(self, shapein=self.M.shapein, direct=self.M.direct, **kwargs)
