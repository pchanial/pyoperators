...
def test_recursion():
    class GaussLegendre1(IterativeAlgorithm):
        def __init__(self, **keywords):
            IterativeAlgorithm.__init__(self, a=1, b=1/np.sqrt(2), t=1/4, p=1,
                p_dtype=int, normal_stop_condition=MaxIterationStopCondition(
                10), **keywords)
        def iteration(self):
    ...
    class GaussLegendre2(IterativeAlgorithm):
        def __init__(self, **keywords):
            IterativeAlgorithm.__init__(self, a=1, b=1/np.sqrt(2), t=1/4, p=1,
                p_dtype=int, allocate_new_state=False,
                normal_stop_condition=MaxIterationStopCondition(10), **keywords)
        def iteration(self):
            self.a_new = (self.a + self.b) / 2
            self.b_new = np.sqrt(self.a * self.b)
            self.t_new = self.t - self.p*(self.a - self.a_new)**2
    ...
    class GaussLegendre3(IterativeAlgorithm):
        def __init__(self, **keywords):
            IterativeAlgorithm.__init__(self, a=1, b=1/np.sqrt(2), t=1/4, p=1,
                p_dtype=int, inplace_recursion=True,
                normal_stop_condition=MaxIterationStopCondition(10), **keywords)
        def iteration(self):
