...
Operators are easy to manipulate: they can be multiplied by a scalar, combined by addition, multiplication, composition or "partition":#partition. The synthax uses the usual arithmetic signs, but be careful that the @*@ sign currently stands for composition (and not multiplication), following the convention for linear operators.
...
|  *multiplication* | @MultiplicationOperator@ | <code>\( A(x) \times B(x) \)</code> |
...
The multiplication is an element-wise multiplication of the outputs of the operands (and not their Hadamard product). The @*@ sign may be confusing when multiplying non-linear operators and currently, the only way to perform element-wise multiplication is to call the composite operator:

{% highlight python %}
>>> MultiplicationOperator([A, B])([2., 2.])
array([ 4.,  8.])
{% endhighlight %}
