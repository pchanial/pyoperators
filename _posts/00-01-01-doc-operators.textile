...
The @Operator@ class and its subclasses are function factories. It means that @Operator@ instances are functions (Python callables) and they take "numpy":http://numpy.scipy.org/ 's N-dimensional array object as inputs and outputs.

As a consequence, to perform computations using an operator class, we first need to create an instance of it:

{% highlight python %}
A = FftOperator(1024)
{% endhighlight %}

The object @A@ is a Python callable, and can be applied over arrays:
...
The operator's data type is a data type object (an instance of "numpy.dtype":http://docs.scipy.org/doc/numpy/reference/arrays.dtypes.html class). It is used to determine the output data type, as being the common type of the operator and input data types, following the standard coercion rules. 
