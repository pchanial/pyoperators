...
.h4 Explicit input shape and explicit output shape.
{% highlight python %}
>>>class Op(Operator):
...
.h4 Unconstrained input shape and explicit output shape.
{% highlight python %}
>>>class Op(Operator):
       def __init__(self):
...
.h4 Explicit input shape and implicit output shape
...
.h4 Unconstrained input shape and implicit output shape
>>>class Op(Operator):
       def __init__(self):
...
.h4 Explicit input shape and unconstrained output shape
>>>class Op(Operator):
       def __init__(self):
...
.h4 Unconstrained input shape and unconstrained output shape
>>>class Op(Operator):
       def __init__(self):
           Operator.__init__(self)
       def direct(self, input, output):
