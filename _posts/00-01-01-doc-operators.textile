...
Operators are easy to manipulate: they can be multiplied by a scalar, combined by addition, composition, elementwise (Hadamard) multiplication or <a href="#partition">partition</a>.
...
The @*@ sign may be confusing when multiplying non-linear operators, so currently, the only way to perform elementwise multiplication is to call the composite operator:
...
Transforming a single-argument http://www.scipy.org/ufunc:@ufunc@ into an @Operator@ is straightforward:
