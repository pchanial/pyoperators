...
{% endhighlight %}
 ...
 It is used to determine the output data type, as being the common type of the operator and input data types, following the standard coercion rules. 
...
The partition along the specified axis is always explicit and is equal to a tuple of ones with as many elements as the number of blocks.

In the following example is shown a stack partition block column operator:
...
The outputs of the blocks are stacked along the dimension specified by @new_axisout@ keyword, i.e the first one.

The following example shows a stack partition block diagonal operator:
...
Block row and block column chunk partition operators can handle implicit partitions:
