...
"In-Place operations":#in-place can be performed in a similar way by providing the input array as the output.
...
Not documented. API will change in version 0.2.
...
Not documented. API might change in version 0.2.
...
h4. <a name="inplace_operators"></a> 2.11. In-place and out-of-place operators
...
Such a property is set by the @inplace@ decorator. For instance, the operator @DiagonalOperator@ is an in-place operator and the following computation is done in-place:
...
it is not possible to avoid the use of a temporary variable, since a buffer diffrent from the @input@ variable is required for the @OUT@ operator.  The application of @IN@ is performed out-of-place because its output is the @input@ variable.




h4. <a name="in-place_reductions"></a> 2.12. In-place reductions
...
Let see how this property can be used to remove the use of intermediate variables. First, let's assume that the operator @o2@ cannot do in-place reductions:
...
* and then added in-place to the @output@ variable.
