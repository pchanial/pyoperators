...
h2. 6.1 <a name="distributionidentityoperator"></a> Using MPIDistributionIdentityOperator
...
The @MPIDistributionIdentityOperator@ is a block column operator whose blocks are the identity and for which the output of each block is handled by an MPI process according to its rank. Hence the MPI reduction is performed by the transpose of this operator (present in @H.T@): a block row operator whose blocks are the identity.
...
h2. 6.2 <a name="distributionglobaloperator"></a> Using MPIDistributionGlobalOperator
